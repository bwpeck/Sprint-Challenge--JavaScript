...
let contactInfo = [];
let infoList= graduates.forEach(function(i){
  contactInfo.push(i.first_name + " " + i.email);
});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
let uni = graduates.filter((includes) => {
return includes.university === "Uni";
});
console.log(uni);
...
let lowerCase = zooAnimals.map((lc) => {
return lc.animal_name.toLowerCase();
});
...
*/
const populationTotal = zooAnimals.reduce((populationT, populationAmount) => {
return populationT + populationAmount.population;
}, 0);
